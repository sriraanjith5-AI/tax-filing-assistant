...
# ============================================================
# Same non-LLM RetrievalMetrics run as tests/test_retrieval_metrics.py,
# just against ChromaVectorStore (a persistent, on-disk ANN index)
# instead of FakeVectorStore (an in-memory brute-force cosine scan),
...
# ============================================================
# 1. LOAD -> CLEAN -> CHUNK -> EMBED -> STORE (in ChromaDB)
# ============================================================
...
# add() is idempotent on chunk_id (re-running against an already
# populated collection just skips duplicates), so assert on the
# collection's total count rather than store_response.total_stored_chunks.
assert vector_store.collection.count() == len(final_chunks), (
    f"ChromaDB collection has {vector_store.collection.count()} chunks, "
    f"expected {len(final_chunks)}."
)
...
csv_path = RESULTS_DIR / f"retrieval_metrics_chromadb_{timestamp}.csv"
results_df.to_csv(csv_path, index=False)

print(f"\n{'=' * 80}")
print("RETRIEVAL METRICS SUMMARY (classical IR metrics, non-LLM, ChromaDB)")
print(f"{'=' * 80}\n")
